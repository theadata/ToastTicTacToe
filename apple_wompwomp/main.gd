...
func _ready():
	...
	now_5 = randf_range(now_4 + 2, 16-1)
	new_game()
...
func check_win():
	...
	if size == 5:
		for i in size:
			...
			diagonal1_sum = grid_data[0][0] + grid_data[1][1] + grid_data[2][2] + grid_data[3][3] + grid_data[4][4]
			diagonal2_sum = grid_data[0][4] + grid_data[1][3] + grid_data[2][2] + grid_data[3][1] + grid_data[4][0]

			#check if either player has all of the markers in one line
			if row_sum == 5 or col_sum == 5 or diagonal1_sum == 5 or diagonal2_sum == 5:
				winner = 1
			elif row_sum == -5 or col_sum == -5 or diagonal1_sum == -5 or diagonal2_sum == -5:
				winner = -1
	if size == 4:
		for i in size:
			row_sum = grid_data[i][0] + grid_data[i][1] + grid_data[i][2] + grid_data[i][3]
			col_sum = grid_data[0][i] + grid_data[1][i] + grid_data[2][i] + grid_data[3][i]
			diagonal1_sum = grid_data[0][0] + grid_data[1][1] + grid_data[2][2] + grid_data[3][3]
			diagonal2_sum = grid_data[0][3] + grid_data[1][2] + grid_data[2][1] + grid_data[3][0]

			#check if either player has all of the markers in one line
			if row_sum == 4 or col_sum == 4 or diagonal1_sum == 4 or diagonal2_sum == 4:
				resize()
			elif row_sum == -4 or col_sum == -4 or diagonal1_sum == -4 or diagonal2_sum == -4:
				resize()
			elif moves == 15:
				resize()
	if size == 3:
